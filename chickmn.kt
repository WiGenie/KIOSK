...
//        같은 변수의 이름을 바꿀 수 있다!! 노가다 멈춰
        ...
        while (true) {
            ...
            for (i in chickenlist) {
                print("<${linejumper + 1}. ${i.ckname} 단품 ${i.ckprice}원>")
                if (++linejumper % 3 == 0) {
                    println()
                } else {
                    print(" ")
            ...
            try {
                ...
                if (choiceck==0) {
                    ...
                    when (singleorset){
                        0 -> continue
                        1 -> {print("뼈를 선택하셨습니다. 수량을 정해주세요: ")
                            ckcount= readLine()!!.toInt()
                            println("${chickenlist[choiceck-1].ckname} 뼈 ${chickenlist[choiceck-1].ckprice} / ${ckcount}개가 주문 목록에 추가되었습니다.")
                            ckorder.add(orderlist("${chickenlist[choiceck-1].ckname} 뼈", chickenlist[choiceck-1].ckprice, ckcount))
                        }
                        2 -> {print("순살을 선택하셨습니다. 수량을 정해주세요: ")
                            ckcount= readLine()!!.toInt()
                            println("${chickenlist[choiceck-1].ckname} 순살 ${chickenlist[choiceck-1].ckprice+boneless} / ${ckcount}개가 주문 목록에 추가되었습니다.")
                            ckorder.add(orderlist("${chickenlist[choiceck-1].ckname} 순살", chickenlist[choiceck-1].ckprice+boneless, ckcount))
                        }
                    }
                    var cktotal:Int=0
                    for (i in ckorder) {
